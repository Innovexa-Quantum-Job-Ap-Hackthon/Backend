...
class CollaborationService:
    ...
    def get_team_jobs(self, team_id: int, days: int = 30) -> List[dict]:
        ...
        if not member_ids:
            logger.info(f"No members found for team {team_id}")
            return []
        ...
        logger.info(f"Found {len(all_jobs)} database jobs for team {team_id} members")
        ...
        for job in all_jobs:
            ...
            if job.user_id:
                ...
                if user:
                    ...
                    logger.info(f"Job {job.jobId}: Found user {user.id} with name='{user_name}' email='{user_email}'")
                else:
                    logger.warning(f"Job {job.jobId}: User {job.user_id} not found in database")
                    user_name = f"User {job.user_id}"
                    user_email = f"user{job.user_id}@unknown.com"
            else:
                logger.warning(f"Job {job.jobId}: No user_id found, using fallback")
            ...
            logger.info(f"Job {job.jobId}: Final job_dict user_name='{user_name}' user_email='{user_email}'")
        ...
        logger.info(f"Returning {len(job_objects)} total jobs (database + IBM) with user info for team {team_id}")
        return job_objects
    ...
    def _fetch_user_ibm_jobs(self, user: models.User, team_creation_time: datetime) -> List[dict]:
        """Fetch IBM jobs for a specific user after team creation time"""
        try:
            from ibm_utils import get_ibm_service_for_user, fetch_ibm_jobs_with_timeout
